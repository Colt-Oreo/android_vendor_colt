...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var COLT_VERSION)
        ZIPFILE=colt-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
...
function coltremote()
{
    ...
    local COLT_USER=$(git config --get review.review.lineageos.org.username)
    if [ -z "$COLT_USER" ]
    then
        git remote add colt ssh://review.lineageos.org:29418/$PFX$GERRIT_REMOTE
    else
        git remote add colt ssh://$LINEAGE_USER@review.lineageos.org:29418/$PFX$GERRIT_REMOTE
    fi
